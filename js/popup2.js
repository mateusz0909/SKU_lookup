...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function result() {
    ...
    let urlWithLanguage = `https://pg.browser.synthrone.io/search?type=pdpd&contains=${input1}&language=${language}${safeUrl}`;
    let urlWithoutLanguage = `https://pg.browser.synthrone.io/search?type=pdpd&contains=${input1}${safeUrl}`;
    ...
    } else if (input1.length !== 0 && language == "empty") {
      Btn.classList.add("running");
      setTimeout(() => {
        window.open(urlWithoutLanguage, "_blank");
        Btn.classList.remove("running");
      }, 1000);
    } else {
